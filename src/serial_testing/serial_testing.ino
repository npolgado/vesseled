...
char incomingByte;
int incomingInt;
bool incoming_meaning = false; // FALSE == POS, TRUE == COLOR
...
void loop()
{
  ...
  if (Serial.available() > 0) {
    ...
    incomingInt = (uint8_t) incomingByte;

    if (incoming_meaning)
    { // color
      mouse_color = CHSV(incomingInt, 255, 255);
    }
    else
    {

      mouse_x = (uint8_t) (incomingInt + (incomingInt * 0.00623268698));
    }

    incoming_meaning = !incoming_meaning;
